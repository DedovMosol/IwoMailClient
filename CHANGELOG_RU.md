...
### Новые функции
- **Проверка обновлений** — кнопка в настройках для проверки и установки новых версий
- **Назначение задач** — возможность назначать задачи другим пользователям с email-уведомлением
- **Приглашения на встречи** — отправка и приём приглашений на события календаря (iCalendar)
- **Приложение по умолчанию** — обработка mailto: ссылок и Share из других приложений
- **Кликабельные контакты** — email и телефоны в календаре, заметках и задачах теперь кликабельные

### Улучшения
- **Оптимизация батареи** — мгновенная реакция на Battery Saver через BroadcastReceiver, увеличен heartbeat PushService
- **Синхронизация после отправки** — папка "Отправленные" обновляется автоматически
- **Фильтрация себя в контактах** — собственный email не отображается в списке организации
- **Изменение учётных данных** — новый пункт в настройках аккаунта
- Оптимизация памяти и производительности (singleton репозитории, батчинг БД, индексы)

### Исправления
- Исправлены потенциальные краши и утечки памяти
- Исправлена бесконечная синхронизация при переключении аккаунтов
- Исправлено сохранение вложений и черновиков при повороте экрана
- Защита от дубликатов в календаре, задачах и подсказках email
...
- Исправлен badge черновиков на главном экране
