...
- **Удаление писем при ошибке сервера** — если сервер не отвечает, письма удаляются локально чтобы не застрять

### Улучшения
- **Заметки для Exchange 2007** — добавлен legacy режим синхронизации заметок через IPM.StickyNote для серверов без поддержки EAS 14.1+
