...
class MailApplication : Application() {
    ...
    companion object {
        ...
        const val CHANNEL_OUTBOX = "outbox"
        const val CHANNEL_SCHEDULED = "scheduled_send"
    ...
    private fun initConscrypt() {
        try {
            ...
            android.util.Log.w(TAG, "Failed to init Conscrypt", e)
        }
    }
    ...
    private fun createNotificationChannels() {
        ...
        // Канал для напоминаний задач - ВЫСОКИЙ приоритет
        if (CHANNEL_TASKS !in existingChannels) {
            val tasksChannel = NotificationChannel(
                CHANNEL_TASKS,
                if (isRussian) "Напоминания задач" else "Task reminders",
                NotificationManager.IMPORTANCE_HIGH
            ).apply {
                description = if (isRussian) "Напоминания о задачах" else "Task reminders"
                enableLights(true)
                enableVibration(true)
                setShowBadge(true)
                lockscreenVisibility = android.app.Notification.VISIBILITY_PUBLIC
        ...
        // Канал для исходящих писем - НИЗКИЙ приоритет
        if (CHANNEL_OUTBOX !in existingChannels) {
            val outboxChannel = NotificationChannel(
                CHANNEL_OUTBOX,
                if (isRussian) "Исходящие" else "Outbox",
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = if (isRussian) "Уведомления об отправке писем из очереди" else "Outbox email notifications"
                setShowBadge(false)
                setSound(null, null)
            }
            notificationManager.createNotificationChannel(outboxChannel)
        }
        
        // Канал для отложенной отправки - ОБЫЧНЫЙ приоритет
        if (CHANNEL_SCHEDULED !in existingChannels) {
            val scheduledChannel = NotificationChannel(
                CHANNEL_SCHEDULED,
                if (isRussian) "Отложенная отправка" else "Scheduled send",
                NotificationManager.IMPORTANCE_DEFAULT
            ).apply {
                description = if (isRussian) "Уведомления об отправке отложенных писем" else "Scheduled email notifications"
                setShowBadge(false)
            }
            notificationManager.createNotificationChannel(scheduledChannel)
        }
        
        // Канал для обновлений приложения - ОБЫЧНЫЙ приоритет
        if (CHANNEL_UPDATE !in existingChannels) {
            val updateChannel = NotificationChannel(
                CHANNEL_UPDATE,
                if (isRussian) "Обновления приложения" else "App updates",
                NotificationManager.IMPORTANCE_DEFAULT
            ).apply {
                description = if (isRussian) "Уведомления о доступных обновлениях" else "Available update notifications"
                setShowBadge(true)
            }
    ...
    private fun scheduleSync() {
        applicationScope.launch(Dispatchers.IO) {
            ...
            val mailRepo = RepositoryProvider.getMailRepository(this@MailApplication)
            mailRepo.initCacheFromDb()
        ...
        applicationScope.launch(Dispatchers.IO) {
            try {
                ...
                val hasExchangePushAccounts = accounts.any { 
                    ...
                    it.syncMode == com.dedovmosol.iwomail.data.database.SyncMode.PUSH.name
                }
                ...
                val minInterval = SyncWorker.getMinSyncIntervalIncludingPush(this@MailApplication)
