...
- 🔄 **EWS для Exchange 2007** — синхронизация заметок через EWS с NTLMv2 (fallback для EAS 12.x)
...
- **Фильтр "Важные"** — новый фильтр для писем с высоким приоритетом
- **Иконка приоритета** — письма с высоким приоритетом отмечаются красным восклицательным знаком
- **Заметки для Exchange 2007** — синхронизация заметок через EWS с NTLMv2 для серверов без EAS 14.1+
- **Синхронизация удалений** — письма, удалённые на сервере, удаляются из локальной БД
- **Параллельная синхронизация** — папки синхронизируются параллельно
