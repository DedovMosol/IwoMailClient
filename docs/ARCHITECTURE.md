...
**Config-change-aware `onDispose` cleanup.** Legacy screens that delete disk resources in `onDispose` (e.g. `SetupScreen` removing orphaned certificate files chosen but not yet saved) guard the deletion with `context.findActivity()?.isChangingConfigurations` (`ui/utils/ContextExtensions.kt`, shared with `AboutScreen`/`SearchScreen`/`AppNavigation`). `MainActivity` declares no `android:configChanges`, so a rotation recreates the Activity and disposes the composition — without the guard the cleanup would delete files that the `rememberSaveable` state still references after recreation, breaking verification/save (SET-1). The check distinguishes a real screen-leave (delete) from a rotation (keep).
