...
6. **Width-adaptive content, not just scaling.** Glance `Row` maps to a non-wrapping horizontal `LinearLayout`, so any content wider than the widget is silently clipped (there is no flow/wrap in RemoteViews). The bottom action row therefore renders **fewer children on narrow sizes** (`size.width >= 300.dp` gate: 4 vs 2 account avatars, last-sync label only when wide), the avatars scale with the shared `scale` factor, and the last-sync label carries `defaultWeight()` + `maxLines=1` so it **truncates** rather than pushing the compose/sync buttons off-screen (robust for any locale/length — the label sits before the buttons in source order, so an unweighted long label would otherwise starve their `AT_MOST` measure). Timestamps use a shared `isSameLocalDay` helper: today → time, otherwise → date (`dd.MM`). Note: `cornerRadius(Dp)` is a no-op below API 31; square corners on Android 8–11 are an accepted trade-off because the drawable-background fallback would reintroduce a known HyperOS launcher crash.
...
**Config-change-aware `onDispose` cleanup.** Composables that tear down state in `onDispose` — `SetupScreen` deleting orphaned certificate files (SET-1), `AppNavigation` clearing `VerificationSecrets` on the verification route (SET-2), `AboutScreen`/`SearchScreen` stopping the easter-egg player — guard the teardown with `context.findActivity()?.isChangingConfigurations` (`ui/utils/ContextExtensions.kt`, one shared `tailrec` helper — the previous private copies were de-duplicated). `MainActivity` declares no `android:configChanges`, so a rotation recreates the Activity and disposes the composition; without the guard the cleanup would destroy resources that the `rememberSaveable` state still references after recreation, breaking verification/save. `findActivity()` (not a raw `as? Activity` cast) is required because `LocalContext.current` is often a `ContextThemeWrapper`, not the Activity itself — a direct cast returns null and silently defeats the guard. The check distinguishes a real screen-leave (tear down) from a rotation (keep).
