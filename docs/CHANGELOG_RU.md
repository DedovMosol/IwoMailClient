...
## v1.6.2 (февраль 2026) — Оптимизация, темы, UI, синхронизация

### Синхронизация и стабильность
- Исправлены ошибки, утечки памяти и race conditions
- Синхронизация стала быстрее: за один запрос загружается вдвое больше писем
- Увеличены таймауты для больших почтовых ящиков (3000+ писем)
- Каждая папка синхронизируется с отдельным таймаутом — одна медленная папка не задерживает остальные
- При обновлении списка загружаются только новые письма
- Ручная синхронизация снижает нагрузку на сервер: одновременно обрабатывается не более двух папок
- Исправлена обработка отмены при синхронизации
- Пометить несколько писем прочитанными можно одним запросом к серверу

### Оформление и интерфейс
- Полоса прокрутки стала ярче и заметнее
- Кнопка «Обновить» в меню письма — перезагружает содержимое с сервера
- Обновлён диалог реквизитов: иконки, кнопка «Скопировать номер счёта», прокрутка и корректное отображение при повороте экрана
- Отправка письма группе контактов — вкладка «Группы» в выборе получателей, при отправке группа раскрывается в адреса всех участников
- Цветовые темы сокращены с 7 до 4 с улучшенной контрастностью
- Обновлённая цветовая гамма главного экрана
- Модернизация карточек главного экрана: Material 3 тени (elevation 2dp), мягкие градиенты, увеличенные иконки, компактный текст
- Пометка черновиков в списке писем: [Локальный] или [Серверный]
- Обновлённая иконка PDF-вложений с надписью «PDF»
- «О приложении» перенесено из настроек в боковое меню
- Виджет масштабирует текст и иконки пропорционально высоте
- Выбор цвета полосы прокрутки (7 вариантов)
- Drag selection во всех папках
- Haptic feedback (вибрация) при выделении и снятии выделения (как в Outlook)
- Обновлён диалог «Темы по дням недели» — темы и цвета скроллбаров разделены на два блока

### Виджет и звуки
- Полностью переработан виджет на рабочем столе: фон из темы приложения, дата, число непрочитанных, ближайшее событие, задачи, заметки, последние письма, аватары аккаунтов, кнопки «Написать» и «Обновить», время последней синхронизации
- Звук при отправке письма

### Календарь и задачи
- Локальная корзина для событий календаря
- Поддержка повторяющихся событий календаря
- Больше информации о задачах в локальной корзине

### Черновики и контакты
- Если сервер не поддерживает серверные черновики — автоматически сохраняются локально (режим можно выбрать в настройках)
- Улучшена проверка email-адреса перед отправкой
- Выбор группы контактов в поле получателя
- При экспорте контактов из глобального справочника проверяются дубликаты с подтверждением

### Вложения
- Долгое нажатие на вложение: «Сохранить» (в папку по умолчанию) или «Сохранить как»

### Календарь (расширение)
- Добавлены все недостающие поля MS-ASCAL: организатор, UID, часовой пояс, исключения повторений, статус встречи, ссылка на онлайн-конференцию, вложения
- Вложения событий календаря: просмотр, скачивание в папку «Загрузки»
- Ссылка на онлайн-конференцию — кликабельна в деталях события

### Безопасность и стабильность
- Пароли больше не передаются через navigation route (Base64) — используется in-memory хранилище с очисткой после использования
- Атомарное создание EasClient через per-account Mutex — исключено двойное создание клиента при параллельных запросах
- HTTP Response body в EWS-клиенте закрывается гарантированно через `use {}` — устранена утечка соединений
- Периодическая перепроверка версии EAS каждые 24 часа (TTL) — корректная обработка обновления Exchange
- Убрано логирование SOAP-ответов и персональных данных (тема, тело заметок, XML-ответы) в release-сборке
- Исправлена потенциальная гонка (ConcurrentModificationException) при остановке Push-соединений в PushService
- Убрана неструктурированная фоновая корутина при добавлении аккаунта — синхронизация календаря/задач теперь через SyncWorker
- Исправлена потеря состояния при повороте экрана — открытые экраны и диалоги больше не закрываются (персонализация, настройки аккаунта, удаление папки и др.)
- Исправлена ошибка компиляции виджета (Intent.data конфликт имени)

### Прочее
- Аудит безопасности, утечек памяти, race conditions и сетевых процессов — результаты в `docs/SECURITY_MEMORY_RACE_NETWORK_AUDIT.md`
- Подготовка к переходу на новый XML-парсер: зафиксирован объём (251 regex), добавлен EasXmlParser, определены этапы миграции по сервисам, правила замены и cleanup-план
- Все изменения совместимы с Exchange 2007 SP1 и новее
