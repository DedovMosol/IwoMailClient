...
## v1.6.2 (февраль 2026) — Кратко

- Синхронизация ускорена и стабилизирована: меньше дублей, корректнее отмена, отдельные таймауты по папкам, batch mark-as-read.
- Исправлены утечки памяти и race conditions (Push/Sync/уведомления), улучшена устойчивость на больших ящиках.
- Обновлён UI: карточки, скроллбар, темы (4 вместо 7), drag-select, haptic feedback, улучшены ключевые диалоги.
- Переработан виджет: актуальные счётчики/события/задачи/последние письма + корректное масштабирование.
- Календарь и задачи: повторяющиеся события, локальная корзина, расширенные поля MS-ASCAL, вложения и онлайн-ссылки в событиях.
- Черновики и контакты: fallback на локальные черновики, выбор групп, проверка email, контроль дублей при экспорте.
- Усилена безопасность: пароли не передаются через route, меньше чувствительного логирования, стабильнее работа EAS/EWS клиентов.
- Добавлен аудит безопасности/памяти/гонок: `docs/SECURITY_MEMORY_RACE_NETWORK_AUDIT.md`.
- Совместимость сохранена: Exchange 2007 SP1+.
