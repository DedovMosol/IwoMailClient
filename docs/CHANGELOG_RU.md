...
### Исправления (27.02.2026)

#### Синхронизация — недосинхронизация папок при первичном sync
- **Проблема:** При первичной синхронизации часть папок не синхронизировалась. Ручная синхронизация внутри одной папки (напр. заметки) вызывала локальную потерю данных в других папках (отправленные).
- **Причина:** `resetAllSyncKeys(accountId)` при Status=3/12 сбрасывал SyncKey **ВСЕХ** папок аккаунта, а не только ошибочной. Это противоречит MS-ASCMD спецификации.
- **Исправлено:** Заменено на `updateSyncKey(folderId, "0")` — сброс только затронутой папки. Убран лишний сброс FolderSync-ключа (`updateFolderSyncKey`).
- **Документация:** [MS-ASCMD Status (Sync)](https://learn.microsoft.com/en-us/openspecs/exchange_server_protocols/ms-ascmd/08151746-faf7-40a3-832b-b42e88a0b729)

#### Задачи — неполные задачи не загружаются с Exchange 2007 SP1
- **Проблема:** Задачи с минимальными полями (без StartDate, DueDate и пр.) не отображались — только полноценные задачи.
- **Причина 1:** Фильтр `ItemClass` отбрасывал элементы, у которых ItemClass не содержит "Task" — но элементы уже из папки Tasks, они задачи по определению.
- **Причина 2:** Fallback-парсинг запускался только при 0 найденных задач, не покрывая частичный парсинг.
- **Причина 3:** `BaseShape=IdOnly` с `task:*` полями — Exchange 2007 SP1 мог возвращать элементы без task-полей.
- **Исправлено:** Убран ItemClass-фильтр. Fallback запускается при несовпадении числа ItemId и спарсенных задач. `AllProperties` BaseShape.

#### Календарь — вложения не загружаются при переустановке и ресинхронизации
- **Проблема:** После сноса приложения и повторной синхронизации вложения у событий не отображались.
- **Причина:** GetItem запрос использовал `AllProperties` без явного `item:Attachments`. Exchange 2007 SP1 может не включать Attachments в AllProperties.
- **Исправлено:** Добавлен `AdditionalProperties` с `item:Attachments` и `item:HasAttachments` в GetItem запрос.
- **Документация:** [AdditionalProperties](https://learn.microsoft.com/en-us/previous-versions/office/developer/exchange-server-2010/aa563810(v=exchg.140))

#### Календарь — неполное удаление событий с сервера
- **Проблема:** При удалении двух событий (в корзину, затем окончательно) одно могло остаться на сервере.
- `deleteEventPermanently` теперь делает best-effort серверное удаление перед локальным (страховка если soft-delete не сработал).
- Batch soft-delete переведён на единый `EasClient` (`deleteEvents`) вместо создания нового клиента для каждого события (устранение дублирования NTLM-хэндшейков и конфликтов SyncKey).
- `deleteEventsWithProgress` теперь логирует ошибки (ранее — `is EasResult.Error -> { }`, полная тишина).
- EWS `ErrorItemNotFound` при удалении теперь логируется как предупреждение для диагностики.

#### Задачи — загрузка только 1 задачи из Exchange 2007 SP1
- **Проблема:** Из всех задач загружалась только одна (с установленными StartDate И DueDate). Сервер возвращал `totalItemsInView=1`.
- **Причина:** `FindItem` с любым BaseShape кроме `IdOnly` (без AdditionalProperties) вызывал неявную серверную фильтрацию на Exchange 2007 SP1.
- **Исправлено:** Двухэтапный подход (по документации MS): `FindItem(IdOnly)` → получаем ВСЕ ItemId, затем `GetItem(AllProperties)` → получаем полные данные задач (Subject, Body, StartDate, DueDate, Status и пр.). Устранена необходимость в отдельном `getTaskBodiesEws`.
- `discoverTaskSubfolderIds` расширен: поиск от `msgfolderroot` (весь ящик) вместо подпапок `tasks`, добавлен `folder:TotalCount` для диагностики.
- **Документация:** [FindItem Operation](https://learn.microsoft.com/en-us/previous-versions/office/developer/exchange-server-2007/aa566107), [GetItem (Task)](https://learn.microsoft.com/en-us/previous-versions/office/developer/exchange-server-2007/aa581352)

#### Календарь — вложения не загружаются после переустановки (окончательное исправление)
- **Проблема:** Предыдущий фикс (AdditionalProperties в GetItem) не помогал, т.к. `syncCalendar()` всегда использует EAS Sync, а `fetchCalendarAttachmentsEws` вызывался только из неиспользуемого EWS-пути.
- **Исправлено:** После EAS sync добавлен `supplementAttachmentsViaEws` — EWS FindItem+CalendarView для обнаружения HasAttachments, затем GetItem для метаданных. Матчинг EAS↔EWS по Subject+StartTime (ID несовместимы). Fallback по Subject для recurring events.

#### Календарь — неточный подсчёт удалений в batch-операциях
- **Проблема:** `deleteCalendarEventsBatchEws` считал ВСЕ элементы удалёнными при наличии хотя бы одного `ResponseClass="Success"` в ответе.
- **Исправлено:** Возвращает точное количество успешных удалений (подсчёт `ResponseClass="Success"` + `ErrorItemNotFound` per-item). Тип возврата `EasResult<Int>` вместо `EasResult<Boolean>`.

#### Онбординг — низкая плавность переходов между слайдами
- **Исправлено:** Добавлен `beyondBoundsPageCount = 1` в HorizontalPager — предзагрузка соседних страниц для мгновенных переходов.

#### Календарь — событие не показывается после создания (ранее)
- Добавлен `RECENT_CREATE_THRESHOLD` (30 сек) в `syncCalendar()` для защиты от преждевременного удаления только что созданных событий.
- Удалён избыточный вызов `syncCalendar()` после создания/обновления события в UI.

#### Календарь — вложения не видны при редактировании (ранее)
- Добавлен `deleteCalendarAttachments` в `EasCalendarService` и `EasClient` (EWS DeleteAttachment).
- `CalendarRepository.updateEvent` принимает `removedAttachmentIds` для открепления вложений.
- `CreateEventDialog` отображает существующие вложения с кнопкой удаления.
- `EventDetailDialog` показывает placeholder если `hasAttachments=true` но метаданные ещё не загружены.

### Главное
- Переработан интерфейс: обновлены ключевые экраны, добавлены новые UI-элементы, кастомные иконки файлов, дополнительные скроллбары и обновлённый виджет.
- Вложения получили полноценные действия **«Сохранить»** и **«Сохранить как»** (в почте и календаре), с более удобным UX.
- Улучшен мультивыбор: поддержка **drag selection**, пакетные операции и более предсказуемое поведение массовых действий.
- Усилена проверка email перед отправкой: меньше ошибок адресации и лишних отказов на стороне сервера.
- Календарь заметно доработан: вложения в событиях, повторяющиеся события, локальная корзина и улучшенная обработка удаления/синхронизации.
- Контакты: добавлена поддержка групп при написании письма, улучшено добавление адресатов и предупреждения о дубликатах.
- Черновики: добавлен переключаемый режим хранения (**локально / сервер**) в настройках аккаунта.
- Exchange 2007 SP1: углублена совместимость EAS/EWS — стабильнее синхронизация задач, вложений календаря и папок.

### Надёжность, производительность и безопасность
- Повышена стабильность Sync/Push: меньше гонок и дублей, лучше обработка таймаутов и ограничение параллельности на больших ящиках.
- Улучшена работа на устройствах с агрессивными ограничениями фона (MIUI/HyperOS/EMUI): критичные действия перенесены в WorkManager.
- Исправлены проблемные места EWS/EAS для Exchange (включая удаление календарных событий, envelope/версии/порядок элементов).
- Снижены риски утечек памяти и лишнего энергопотребления, улучшена обработка состояний UI при поворотах и повторных открытиях.
- Усилена защита данных синхронизации и общая устойчивость приложения в нестабильных сетевых сценариях.
