...
### Новое
- Подготовка к миграции парсинга: добавлен `EasXmlParser.kt` (drop-in API + DSL + namespaces) и план миграции `docs/XMLPULLPARSER_MIGRATION_PLAN.md`.
- Добавлена пасхалка в поиске (overlay с анимацией/звуком/вибрацией).

### Улучшения
- Sync/Push/надёжность: windowSize 100→200, per-folder таймауты, ограничение параллельности (до 2 папок), инкрементальный pull-to-refresh.
- Почта и контакты: фоновая догрузка тела и FileReference-вложений, batch-пометка «прочитано», отправка группе контактов, валидация email и экспорт GAL без дублей.
- Календарь и черновики: корзина календаря/повторы, выбор режима черновиков, действия «Сохранить»/«Сохранить как» для вложений.
- Интерфейс и темы: 4 темы (WCAG AA) с автомиграцией, drag selection + haptic, улучшения тулбара письма, скроллбаров и виджета.
- Уведомления: добавлены action-кнопки в календаре («Прочитано», «Напомнить через 5 минут»), задачах и почте («Прочитано»).
- Надёжность под OEM-ограничениями (MIUI/HyperOS/EMUI): server-side действия из notification receiver перенесены в WorkManager (mark read/mark complete).

### Исправления
- EWS: исправлены SOAP envelope, SendMeetingCancellations, RequestServerVersion и порядок элементов.
- Календарь (Exchange 2007 SP1+): устранено «воскрешение» удалённых событий после синхронизации (уточнена серверная семантика удаления для EAS/EWS, усилен разбор ответов Sync и retry-обработка ошибок).
- Защита данных синхронизации: многоуровневая защита календаря от потери событий и защита тела письма от затирания пустым ответом сервера.
- UI state: фиксы поворота экрана (navigation/dialog state, saveable ID), фикс PendingIntent дедупликации и двойного тапа в виджете.
- Автоочистка: исправлен учёт успешного прохода — `lastTrashCleanupTime` обновляется только при полностью успешной очистке; при частичных ошибках включается retry через WorkManager.
- Автоочистка (Trash/Drafts/Spam): изменена семантика «раз в N дней» — теперь выполняется полная очистка папок по интервалу, а не удаление писем старше N дней.
- Алерты на экранах контактов/календаря/задач/заметок/черновиков: унифицированы тексты кнопок подтверждения на «Да/Нет».

### Производительность
- Меньше дублей/race conditions в Sync/Push, стабильнее уведомления в foreground.
- Повышена устойчивость на больших ящиках за счёт таймаутов и ограничения параллельности.

### Совместимость и безопасность
- Повторно подтверждена корректность EAS/EWS для Exchange 2007 SP1+.
- Усилена обработка чувствительных данных.
